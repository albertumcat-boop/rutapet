/**
 * DashboardScreen.jsx — Pantalla principal
 * Auditado: datos reales Firebase, notificaciones funcionales, responsive
 */
...
export default function DashboardScreen({ nav }) {
  ...
  const notificaciones = [
    ...inact.map(c => ({
      ...
      desc:    `Hace ${daysSince(c.ultimaVisita)} días sin visita`,
    ...
    <div className="screen-enter" style={{ background:C.gray50, minHeight:'100vh' }}>
      ...
      <div style={{ background:C.navy, padding:'20px 16px 28px', color:'#fff' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
          <div style={{ minWidth:0, flex:1 }}>
            <p style={{ fontSize:11, color:C.gray400, margin:0 }}>{empresa}</p>
            ...
            <div style={{ position:'relative' }}>
              ...
              {showNotif && (
                <>
                  ...
                  <div style={{ position:'absolute', top:36, right:-8, width:290, maxWidth:'85vw', background:'#fff', borderRadius:16, boxShadow:'0 8px 32px rgba(0,0,0,0.18)', zIndex:99, overflow:'hidden', border:`1px solid ${C.gray200}` }}>
                    <div style={{ padding:'12px 14px', borderBottom:`1px solid ${C.gray200}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      <span style={{ fontSize:13, fontWeight:800, color:C.gray800 }}>Notificaciones</span>
                      <span style={{ fontSize:11, color:C.teal, fontWeight:700 }}>{notificaciones.length} alertas</span>
                    </div>
                    {notificaciones.length === 0 ? (
                      ...
                      <div style={{ maxHeight:300, overflowY:'auto' }}>
                        {notificaciones.map((n, i) => (
                          <div key={n.id} onClick={n.onClick}
                            style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 14px', borderBottom:i<notificaciones.length-1?`1px solid ${C.gray100}`:'none', cursor:'pointer', background:'#fff' }}
      ...
      {inact.length > 0 && (
        <div style={{ padding:'0 14px 14px' }}>
          <p style={{ fontSize:13, fontWeight:800, color:C.gray800, marginBottom:8 }}>⚠️ Sin visita reciente</p>
