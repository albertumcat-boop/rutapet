...
export default function OnboardingScreen() {
  ...
  const addTipo = () => { if(tipos.length>=4)return; setTipos(ts=>[...ts,{key:`t${Date.now()}`,label:'',icon:'users',color:'#94A3B8'}]) }
  const addCat  = () => { if(cats.length>=4)return;  setCats(cs=>[...cs,{key:`c${Date.now()}`,label:'',icon:'pkg',color:'#94A3B8'}])  }
    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:28 }}>
      {[1,2,3].map(n=>(
        <div key={n} style={{ display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:32, height:32, borderRadius:'50%', background:paso>=n?C.teal:C.gray200, display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:14, color:paso>=n?'#fff':C.gray400, transition:'all 0.2s' }}>
        ...
        <div style={{ background:'#fff', borderRadius:24, padding:'24px 20px' }}>
          ...
          {paso===1 && (
            <>
              ...
              <input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Ej: Distribuidora García" style={{ ...inputStyle, marginBottom:14 }} />
              ...
              {rubro==='Otro' && (
                <input value={rubroCustom} onChange={e=>setRubroCustom(e.target.value)} placeholder="Describe tu rubro..." style={{ ...inputStyle, marginBottom:14 }} />
              )}
          ...
          {paso===2 && (
            <>
                ...
                {tipos.map((t,i)=>(
                  <div key={t.key} style={{ background:C.gray50, borderRadius:14, padding:'14px', marginBottom:10, border:`1px solid ${C.gray200}` }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                      ...
                      {tipos.length>1 && <button onClick={()=>rmTipo(i)} style={{ background:'none', border:'none', cursor:'pointer', color:C.red, fontSize:12, fontWeight:700, fontFamily:'inherit' }}>Eliminar</button>}
                    </div>
              ...
              {tipos.length<4 && (
                <button onClick={addTipo} style={{ width:'100%', padding:'11px', borderRadius:12, border:`1.5px dashed ${C.teal}`, background:C.teal+'08', cursor:'pointer', fontFamily:'inherit', fontSize:13, fontWeight:700, color:C.teal, marginBottom:16, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
                  <Icon name="plus" size={15} color={C.teal} /> Agregar tipo
                </button>
              )}
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                <Button variant="ghost" fullWidth onClick={()=>setPaso(1)}>Atrás</Button>
                <Button size="lg" fullWidth icon="chevron" disabled={tipos.filter(t=>t.label.trim()).length===0} onClick={()=>setPaso(3)}>Siguiente</Button>
              </div>
            </>
          )}
          ...
          {paso===3 && (
            <>
                ...
                {cats.map((cat,i)=>(
                  <div key={cat.key} style={{ background:C.gray50, borderRadius:14, padding:'14px', marginBottom:10, border:`1px solid ${C.gray200}` }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                      ...
                      {cats.length>1 && <button onClick={()=>rmCat(i)} style={{ background:'none', border:'none', cursor:'pointer', color:C.red, fontSize:12, fontWeight:700, fontFamily:'inherit' }}>Eliminar</button>}
                    </div>
              ...
              {cats.length<4 && (
                <button onClick={addCat} style={{ width:'100%', padding:'11px', borderRadius:12, border:`1.5px dashed ${C.teal}`, background:C.teal+'08', cursor:'pointer', fontFamily:'inherit', fontSize:13, fontWeight:700, color:C.teal, marginBottom:16, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
                  <Icon name="plus" size={15} color={C.teal} /> Agregar categoría
                </button>
              )}
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                <Button variant="ghost" fullWidth onClick={()=>setPaso(2)}>Atrás</Button>
                <Button size="lg" fullWidth icon="ok_circle" disabled={cats.filter(c=>c.label.trim()).length===0} onClick={handleFinalizar}>¡Listo!</Button>
              </div>
            </>
          )}
        </div>
