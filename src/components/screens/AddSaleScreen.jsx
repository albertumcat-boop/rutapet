...
import { CLIENTES, PRODUCTOS } from '../../constants/data'
...
export default function AddSaleScreen({ onBack, initCId }) {
  ...
  const total   = items.reduce((s,it)=>{const p=PRODUCTOS.find(x=>x.id===it.pId);return s+(p?p.precio*it.qty:0)},0)
    ...
    <div className="screen-enter" style={{ background:C.gray50, minHeight:'100vh' }}>
      ...
      <div style={{ padding:14 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, background:C.gray100, borderRadius:8, padding:'4px 10px' }}>
                      <button onClick={()=>updItem(idx,'qty',Math.max(1,it.qty-1))} style={{ background:'none', border:'none', cursor:'pointer', display:'flex' }}><Icon name="minus_c" size={18} color={C.teal} /></button>
                      <span style={{ fontSize:15, fontWeight:800, minWidth:20, textAlign:'center' }}>{it.qty}</span>
                      <button onClick={()=>updItem(idx,'qty',it.qty+1)} style={{ background:'none', border:'none', cursor:'pointer', display:'flex' }}><Icon name="plus_c" size={18} color={C.teal} /></button>
                    </div>
                    <span style={{ fontSize:14, fontWeight:800, color:C.teal }}>{p?fmtUSD(p.precio*it.qty):'$0.00'}</span>
                  </div>
                </div>
                {items.length>1&&<button onClick={()=>rmItem(idx)} style={{ background:'none', border:'none', cursor:'pointer', padding:4, display:'flex' }}><Icon name="x_circle" size={16} color={C.red} /></button>}
              </div>
            </Card>
          )
        })}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:16 }}>
          {METODOS.map(m=>(
            <button key={m.key} onClick={()=>setMet(m.key)} style={{ padding:'10px 6px', borderRadius:12, border:`2px solid ${metodo===m.key?C.teal:C.gray200}`, background:metodo===m.key?C.teal+'12':'#fff', cursor:'pointer', fontFamily:'inherit', textAlign:'center' }}>
        ...
        <Button icon="ok_circle" size="lg" fullWidth disabled={!cId||total===0} onClick={()=>{setDone(true);setTimeout(onBack,1800)}}>Registrar venta</Button>
      </div>
      <div style={{ height:90 }} />
    </div>
  )
