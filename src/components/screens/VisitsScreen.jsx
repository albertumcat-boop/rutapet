...
export default function VisitsScreen({ nav, onBack }) {
  const { clientes, visitas: visitasData, recargar } = useAppData()
  ...
  const conversion = visitasData.length > 0
    ? Math.round((visitasData.filter(v => v.vendio).length / visitasData.length) * 100)
    : 0
    ...
    <div className="screen-enter" style={{ background:C.gray50, minHeight:'100vh' }}>
      ...
      <div style={{ padding:14 }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
          <KpiCard label="Total visitas"  val={visitasData.length}   icon="calendar" color={C.teal}  />
        ...
        {visitasData.length === 0 ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <p style={{ fontSize:13, color:C.gray400, marginTop:8 }}>Sin visitas registradas aún</p>
          </div>
        ) : visitasData.slice(0,10).map(v => {
        ...
        {pendientes.length === 0 ? (
          <div style={{ textAlign:'center', padding:'16px 0' }}>
            ...
            <p style={{ fontSize:13, color:C.gray400, marginTop:8 }}>¡Todos los clientes visitados recientemente!</p>
          </div>
        ) : pendientes.map(c => (
          <Card key={c.id} onClick={() => nav('clientDetail', c)}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <p style={{ fontSize:12, color:C.red, fontWeight:700, margin:0 }}>
