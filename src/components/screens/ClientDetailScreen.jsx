...
import { actualizarCliente, eliminarCliente } from '../../services/firestore'
...
export default function ClientDetailScreen({ cliente, onBack, nav }) {
  const { clientes, ventas, visitas, recargar } = useAppData()
              ...
              <div style={{ display:'flex', gap:8, marginBottom:14 }}>
                {['alto','medio','bajo'].map(n => (
                  <button key={n} onClick={() => upd('nivel', n)}
                    style={{ flex:1, padding:'8px', borderRadius:10, border:`2px solid ${form.nivel===n?C.teal:'#E2E8F0'}`, background:form.nivel===n?C.teal+'12':'#fff', cursor:'pointer', fontFamily:'inherit', fontSize:12, fontWeight:700, color:C.gray800 }}>
