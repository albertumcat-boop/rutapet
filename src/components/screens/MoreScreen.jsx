 ...
 * MoreScreen.jsx — Menú principal
 * Auditado: datos reales, sin datos demo
 */
import { C } from '../../constants/colors'
import { useAppData } from '../../hooks/useAppData'
import { useConfig } from '../../context/ConfigContext'
    ...
    <div className="screen-enter" style={{ background:C.gray50, minHeight:'100vh' }}>
      ...
      <div style={{ background:C.navy, padding:'20px 14px 24px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:16 }}>
          <Avatar initials={avatar} size={52} />
          <div style={{ minWidth:0, flex:1 }}>
            <p style={{ fontSize:16, fontWeight:800, color:'#fff', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {nombre}
            </p>
            <p style={{ fontSize:11, color:C.gray400, margin:'2px 0' }}>{user?.email || ''}</p>
            <Badge bg={C.teal+'30'} txt={C.teal}>{empresa}</Badge>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
          {[
            { l:'Ventas $',   v:fmtUSD(total)   },
            { l:'Clientes',   v:clientes.length  },
            { l:'Ventas #',   v:ventas.length    },
          ].map((k,i) => (
      ...
      <div style={{ padding:14 }}>
          ...
          <Card key={i} onClick={() => nav(item.screen)}>
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:14, fontWeight:700, color:C.gray800, margin:0 }}>{item.label}</p>
                <p style={{ fontSize:11, color:C.gray400, margin:0 }}>{item.sub}</p>
              </div>
        ...
        <div style={{ background:'#DCFCE7', borderRadius:14, padding:'12px 14px', display:'flex', alignItems:'center', gap:10, marginTop:4 }}>
          ...
          <div>
            <p style={{ fontSize:12, fontWeight:700, color:'#166534', margin:0 }}>Conectado — Firebase activo</p>
            <p style={{ fontSize:11, color:'#15803D', margin:0 }}>{config.empresa?.rubro || 'Ventas en ruta'}</p>
          </div>
        </div>

        {/* Logout */}
        <button onClick={onLogout}
          style={{ width:'100%', marginTop:14, padding:'13px', background:'#FEE2E2', border:'none', borderRadius:14, color:C.red, fontSize:14, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, fontFamily:'inherit' }}>
    ...
    <div className="screen-enter" style={{ background:C.gray50, minHeight:'100vh' }}>
      ...
      <div style={{ background:C.navy, padding:'20px 14px 24px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          <Avatar initials={avatar} size={52} />
          <div>
            <p style={{ fontSize:17, fontWeight:800, color:'#fff', margin:0 }}>{nombre}</p>
            <p style={{ fontSize:12, color:C.gray400, margin:'2px 0' }}>{user?.email}</p>
            <Badge bg={C.teal+'30'} txt={C.teal}>{config.empresa.nombre || 'Mi Empresa'}</Badge>
          </div>
        </div>
      ...
      <div style={{ padding:14 }}>
        ...
        <div style={{ background:'#DCFCE7', borderRadius:14, padding:'12px 14px', display:'flex', alignItems:'center', gap:10, marginTop:4 }}>
          <Icon name="wifi" size={18} color={C.green} />
          <div>
            <p style={{ fontSize:13, fontWeight:700, color:'#166534', margin:0 }}>Conectado — datos en Firebase</p>
            <p style={{ fontSize:11, color:'#15803D', margin:0 }}>{config.empresa.rubro || 'Ventas en ruta'}</p>
          </div>
        </div>
        ...
        {/* Logout */}
        <button onClick={onLogout}
          style={{ width:'100%', marginTop:14, padding:'13px', background:'#FEE2E2', border:'none', borderRadius:14, color:C.red, fontSize:15, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, fontFamily:'inherit' }}>
          <Icon name="logout" size={18} color={C.red} />
          Cerrar sesión
        </button>
      </div>
